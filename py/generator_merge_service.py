...
class CandidateGeneratorService:
    ...
    def __init__(
        ...
        self.uv_dict = {}
        self.num_lsr_sent = 0
        self.items_waiting_for_lsr = []  # noqa List to store items waiting to be sent to late stage ranker
        self.already_sent = {}  # noqa Dictionary to track items that have already been sent
    ...
    def OnGeneratorCompletion(
        ...
        for result in results:
            ...
            if uv_estimate > self.lsr_sufficiency_threshold and item_id not in self.already_sent:
                self.enqueue_for_late_stage_ranker(item_id)

    def enqueue_for_late_stage_ranker(self, item_id):
        if self.num_lsr_sent < self.max_num_lsr and item_id not in self.items_waiting_for_lsr:
    ...
    def OnTimeOut(self):
        ...
        top_K_items = [(item_id, value) for item_id, value in top_K_items if item_id not in self.already_sent]
...
candidate_generator_service = CandidateGeneratorService(
    max_num_lsr, lsr_sufficiency_threshold, max_num_esr, 
    lsr_batch_size, weights
)

# Example 1
generator_id_1 = 1
results_1 = [GeneratorResult("item_1", 1, 0.9), GeneratorResult("item_2", 2, 0.8), GeneratorResult("item_3", 3, 0.7)]
candidate_generator_service.OnGeneratorCompletion(generator_id_1, results_1)

# Example 2
generator_id_2 = 2
results_2 = [GeneratorResult("item_4", 1, 0.85), GeneratorResult("item_5", 2, 0.78), GeneratorResult("item_6", 3, 0.72)]
